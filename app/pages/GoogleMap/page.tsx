...
//     const volunteerName = use(searchParams);
...
//       handleAutoSearch(volunteerName as string);
//     }
...
//           Locating {volunteerName as string}...
//         </div>
//       )}
...
//                 <h2 className="text-5xl font-black italic uppercase">{volunteerName as string}</h2>
      ...
      <AnimatePresence>
        {showDetails && maplocation && (
          <motion.div
            ...
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 20, opacity: 0 }}
            className="w-full max-w-5xl bg-white text-black p-8 mt-8 rounded-[2.5rem] border-[6px] border-white shadow-2xl"
          >
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="text-center md:text-left">
                <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs">Volunteer Found</p>
                <h2 className="text-5xl font-black italic uppercase">{maplocation.name || "Volunteer"}</h2>
                <p className="mt-2 text-zinc-400">Viewing current selected location.</p>
              </div>
